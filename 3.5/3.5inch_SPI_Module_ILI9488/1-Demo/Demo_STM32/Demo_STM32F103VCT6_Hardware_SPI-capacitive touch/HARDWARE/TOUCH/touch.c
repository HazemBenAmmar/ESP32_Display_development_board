...
{			
    u8  uu8=0;
    GT911_POINT_REG	GT911_POINTS;
    //delay_ms(10);
	if(PEN==0)//有按键按下
	{
	    //delay_ms(10);
        I2CGT911_Read(0X814E,&uu8,1);	
		if(uu8&0x80)  //数据准备好
		{		
			uu8 &= 0x0F;
			if(uu8>=1)
			{		
				I2CGT911_Read(0x814F,(u8 *)(&GT911_POINTS),sizeof(GT911_POINT_REG));	
                tp_dev.x = (((u16)GT911_POINTS.xh)<<8) + GT911_POINTS.xl;
				tp_dev.y = (((u16)GT911_POINTS.yh)<<8) + GT911_POINTS.yl;				
				tp_dev.x &= 0x0FFF;
				tp_dev.y &= 0x0FFF;
            }
			uu8 = 0;
            I2CGT911_Write(0X814E,&uu8,1);	
        }
		if((tp_dev.sta&TP_PRES_DOWN)==0)//之前没有被按下
		{		 
			tp_dev.sta=TP_PRES_DOWN|TP_CATH_PRES;//按键按下  
...
void CTP_Adjust(void)
{
    LCD_Clear(WHITE);//清屏
    LCD_ShowString(35,110,lcddev.width,lcddev.height,16,"CTP No Touch Screen Adjust!");//校正完成
	delay_ms(1000);
    LCD_Clear(WHITE);//清屏   
	return;//校正完成		
}
